...
impl Orderbook {
    ...
    pub fn retrieve_level(&self, level_price: &f64) -> Result<Level, LevelError> {
        ...
        if let Ok(i_level) = self.find_level(level_price) {
            ...
            if i_level < 0 {
                ...
                Ok(self.bids[i_level as usize].clone())

            // Level is on the Ask side
            } else if i_level > 0 {
                let i_level = i_level - 1;
                Ok(self.asks[i_level as usize].clone())

            // level is not present
            } else {
                Err(LevelError::LevelInfoNotAvailable)
            }
    ...
    pub fn delete_level(&mut self, level_price: &f64) -> Result<(), LevelError> {
        ...
        match find_level_ob {
            Ok(n) if n < 0 => {
                let bid_found = find_level_ob.unwrap().unsigned_abs() as usize - 1;
    ...
    pub fn find_order(
        ...
        match find_level_ob {
            Ok(n) if n < 0 => {
                let level_found = find_level_ob.unwrap().unsigned_abs() as usize - 1;
    ...
    pub fn retrieve_order(&self, price: f64, order_ts: u64) -> Result<Order, OrderError> {
        if let Ok((found_level, found_order)) = self.find_order(price, order_ts) {
            if found_level > 0 {
                Ok(self.asks[found_level.unsigned_abs() as usize].orders[found_order])
            } else {
                Ok(self.bids[found_level.unsigned_abs() as usize].orders[found_order])
            }
        } else {
            Err(OrderError::OrderNotFound)
        }
    ...
    pub fn delete_order(&mut self, price: f64, order_ts: u64) -> Result<(), OrderError> {
        if let Ok((found_level, found_order)) = self.find_order(price, order_ts) {
            if found_level > 0 {
                self.asks[found_level.unsigned_abs() as usize - 1]
                    .orders
                    .remove(found_order);

                Ok(())
            } else {
                self.bids[found_level.unsigned_abs() as usize - 1]
                    .orders
                    .remove(found_order);
                Ok(())
            }
        ...
        match self.find_order(price, order_ts) {
            Ok((found_level, found_order)) => {
                if found_level < 0 {
                    println!(
                        "\nfounded level: {:?}",
                        found_level.unsigned_abs() as usize - 1
                    );

                    let founded_order = self.bids[found_level.unsigned_abs() as usize]
                        .orders[found_order];
                    ...
                    let founded_order = self.asks[found_level.unsigned_abs() as usize]
                        .orders[found_order];
